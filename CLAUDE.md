...
Project conventions for **Unbroken** — a tiny Android app that counts the
...
- State: one `AndroidViewModel` exposing `StateFlow<UnbrokenUiState>`
...
- Tests: JVM unit tests under **Robolectric 4.16.1** (in-memory
  ...
  Conventions in `app/src/test/java/com/github/reygnn/unbroken/TESTING_CONVENTIONS.kt`.
...
The whole app lives in **one file**: `app/src/main/java/com/github/reygnn/unbroken/MainActivity.kt`.
It contains, in order:

- `UnbrokenUiState` — immutable snapshot consumed by the UI.
- `UnbrokenViewModel` — owns `SharedPreferences`, computes the state.
- `MainActivity` — Compose entry point.
- `UnbrokenTheme` — Material 3 + dynamic color wrapper.
- `UnbrokenScreen` + `StartDatePickerDialog` — Composables.
...
1. **Logic in the ViewModel, not in the Composable.** Date math, date
   ...
   `UnbrokenViewModel`. Composables read `StateFlow` and render. This
...
4. **Testable logic lives outside Activity / Composable bodies.**
   ...
   decisions) belongs in `UnbrokenViewModel`. The Composable becomes thin
...
5. **New comments and KDoc are English; existing German UI strings stay.**
   Code comments and KDoc you add or rewrite are written in English. The
   brand ("Unbroken") is English in both label and title bar. The rest of
   the user-facing strings (`"Seit dem"`, `"Aktualisieren"`, `"Abbrechen"`)
   are German and stay German — this is a personal-use app for a German
   speaker. If the app ever gets localised, strings move to
